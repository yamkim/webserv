...
bool HTTPRequestHandler::getHeaderStartLine(void) {
    ...
    chunk = getStringHeadByDelimiter(_buffer, delimiterLength, " ");
...
bool HTTPRequestHandler::getHeader(void) {
    ...
    key = getStringHeadByDelimiter(_buffer, delimiterLength, ": ");
    if (key.length() == 0) {
        throw ErrorHandler("Error: HTTP Header error.", ErrorHandler::ALERT, "HTTPRequestHandler::getHeader");
    }
    if (_buffer.find("\r\n") == std::string::npos) {
        value = getStringHeadByDelimiter(_buffer, delimiterLength, "\n");
    } else {
        value = getStringHeadByDelimiter(_buffer, delimiterLength, "\r\n");
    }
    _headers[key] = value;
