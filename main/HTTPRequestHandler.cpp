...
HTTPRequestHandler::HTTPRequestHandler(int connectionFd) {
    // REVIEW : accept에서 클라이언트의 IP와 포트도 받을 수 있도록 하면 좋을것 같습니다.

    _phase = new Phase(PARSE_STARTLINE);
    _contentLength = new long(-1);
    _fileController = NULL;
    _connectionFd = new int(connectionFd);
    _headerString = new std::string("");
    _headers = new std::map<std::string, std::string>;
    _requestLine = new std::string("");
}
